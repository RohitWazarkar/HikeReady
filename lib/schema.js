...
export const enums = {
  ...
  ContactSubject: ["General", "Bug", "Feedback", "Content", "Partnership"],
};
...
export const models = {
  // Top-level areas of the app (Interview Questions, MCQ, Resume Builder,
  // Tutorials). Each category (subject) belongs to a section.
  sections: {
    file: "sections.json",
    prefix: "sec",
    fields: {
      ...
      // "questions" | "mcq" | "resume" | "tutorials" — how content behaves.
      kind: { type: "string", default: "questions" },
  ...
  // A domain sits between a section and its subjects. e.g. under Interview
  // Questions: "IT" and "Medical Coding". Categories (subjects) belong to a
  // domain. Optional for back-compat: categories with no domainId are treated
  // as the "IT" domain of the interview section.
  domains: {
    file: "domains.json",
    prefix: "dom",
    fields: {
    ...
    unique: ["slug"],
    indexes: ["slug", "sectionId"],
    relations: {
      sectionId: { model: "sections", onField: "id" },
    },
  },
  ...
  categories: {
    ...
    fields: {
      ...
      slug: { type: "string", required: true },
      description: { type: "string" },
      // Which section this subject belongs to. Optional for back-compat:
      // categories without it are treated as the "interview" section.
      sectionId: { type: "string" },
      // Which domain (e.g. IT / Medical Coding) this subject belongs to.
      // Optional for back-compat: no domainId is treated as the "IT" domain.
      domainId: { type: "string" },
    },
    unique: ["slug", "name"],
    indexes: ["slug", "sectionId", "domainId"],
    relations: {
      // Only enforced when sectionId is present.
      sectionId: { model: "sections", onField: "id" },
      domainId: { model: "domains", onField: "id" },
    },
  },
  ...
  // Contact form submissions (stored in JSON for now).
  contactMessages: {
    ...
    fields: {
      name: { type: "string", required: true },
      email: { type: "string", required: true },
      subject: { type: "enum", enum: "ContactSubject", default: "General" },
      message: { type: "string", required: true },
    },
    unique: [],
    indexes: ["email", "subject"],
    relations: {},
  },
};
