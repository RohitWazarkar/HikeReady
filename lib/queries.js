...
// The default section a category belongs to when it has no sectionId (back-
// compat: all pre-existing content is Interview Questions).
export const INTERVIEW_SECTION_ID = "sec_interview";

// Resolve a category's effective sectionId (defaults to interview).
const sectionIdOf = (category) => category.sectionId || INTERVIEW_SECTION_ID;
...
// A single section by slug (null if not found).
export async function getSectionBySlug(slug) {
  return db.sections.findUnique({ slug });
}

// --- Domains (a layer between a section and its subjects) -------------------

// The default domain a category belongs to when it has no domainId (back-compat:
// all pre-existing subjects are IT).
export const IT_DOMAIN_ID = "dom_it";
const domainIdOf = (category) => category.domainId || IT_DOMAIN_ID;

// Domains within a section (e.g. IT, Medical Coding under Interview), ordered.
export async function getDomainsForSection(sectionId) {
  const domains = await db.domains.findMany({ where: { sectionId } });
  return [...domains].sort(byOrder);
}

// A single domain by slug (null if not found).
export async function getDomainBySlug(slug) {
  return db.domains.findUnique({ slug });
}

// Subjects (categories) that belong to a domain, with topic + question counts
// and difficulty spread. Categories with no domainId are treated as IT.
export async function getCategoriesForDomain(domainId) {
  ...
  return categories
    .filter((c) => domainIdOf(c) === domainId)
    .map((c) => {
...
// Categories (subjects) that belong to a section, with topic + question counts
// and a difficulty spread — same shape the practice cards expect. Categories
// with no sectionId are treated as the interview section.
export async function getCategoriesForSection(sectionId) {
  const [categories, topics, questions] = await Promise.all([
    db.categories.findMany(),
    db.topics.findMany(),
    db.questions.findMany({ where: { status: "Published" } }),
  ...
  return categories
    .filter((c) => sectionIdOf(c) === sectionId)
    .map((c) => {
      const catTopics = topics.filter((t) => t.categoryId === c.id);
      const catTopicIds = new Set(catTopics.map((t) => t.id));
      const catQuestions = questions.filter((q) => catTopicIds.has(q.topicId));
...
// Full navigation tree for the sidebar: categories with their topics nested.
// Optionally filtered to a section; defaults to the Interview section so the
// existing sidebar keeps showing interview subjects.
// Shape: [{ id, name, slug, topics: [{ id, name, slug }] }]
export async function getNavTree(sectionId = INTERVIEW_SECTION_ID) {
  const [categories, topics] = await Promise.all([
    db.categories.findMany(),
    db.topics.findMany(),
  ]);
  return categories
    .filter((c) => !sectionId || sectionIdOf(c) === sectionId)
    .map((c) => ({
      id: c.id,
      name: c.name,
      slug: c.slug,
